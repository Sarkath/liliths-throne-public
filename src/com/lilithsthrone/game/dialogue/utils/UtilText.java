...
//					System.err.println("Parsing error: Could not initialise npc 2");
			...
			for(int i = 0; i<npcs.size(); i++) {
				if(i==0) {
					api.put("ncom", npcs.get(i));
				}
				api.put("ncom"+(i+1), npcs.get(i));
			}
		}
	...
	public static void initScriptEngine() {
		...
		if(ctx == null) {
			...
			api = new HashMap<>();
			api.put("game", Main.game);
			api.put("sex", Main.sex);
			api.put("combat", Main.combat);
			api.put("properties", Main.getProperties());
			api.put("RND", Util.random);
			api.put("itemGem", Main.game.getItemGen());
		}
			...
			for(ParserTarget target : ParserTarget.values()) {
				if(target!=ParserTarget.STYLE && target!=ParserTarget.UNIT && target!=ParserTarget.NPC && target!=ParserTarget.COMPANION && target!=ParserTarget.NON_COMPANION) {
					for(String tag : target.getTags()) {
						api.put(tag, target.getCharacter(tag, null));
					}
				}
			}
...
//		}
		...
		for(DayOfWeek dayOfWeek : DayOfWeek.values()) {
			api.put("DOW_"+dayOfWeek, dayOfWeek);
		}

		// Items:
		for(AbstractWeaponType weaponType : WeaponType.getAllWeapons()) {
			api.put("WEAPON_"+WeaponType.getIdFromWeaponType(weaponType), weaponType);
		}
		for(AbstractClothingType clothingType : ClothingType.getAllClothing()) {
			api.put("CLOTHING_"+ClothingType.getIdFromClothingType(clothingType), clothingType);
			api.put("CT_"+ClothingType.getIdFromClothingType(clothingType), clothingType);
		}
		for(AbstractItemType itemType : ItemType.getAllItems()) {
			api.put("ITEM_"+ItemType.getIdFromItemType(itemType), itemType);
		}
		for(AbstractSetBonus setBonus : SetBonus.getAllSetBonuses()) {
			api.put("SET_BONUS_"+SetBonus.getIdFromSetBonus(setBonus), setBonus);
		}
		for(ItemTag it : ItemTag.values()) {
			api.put("ITEM_TAG_"+it.toString(), it);
		}
		for(CoverableArea ca : CoverableArea.values()) {
			api.put("CA_"+ca.toString(), ca);
		}
		for(InventorySlot is : InventorySlot.values()) {
			api.put("IS_"+is.toString(), is);
		}

		// Misc.:
		for(Colour colour : PresetColour.getAllPresetColours()) {
			api.put("COLOUR_"+PresetColour.getIdFromColour(colour), colour);
		}

		// Bodies:
		for(AbstractRace race : Race.getAllRaces()) {
			api.put("RACE_"+Race.getIdFromRace(race), race);
		}
		for(AbstractRacialBody racialBody : RacialBody.getAllRacialBodies()) {
			api.put("RACIAL_BODY_"+RacialBody.getIdFromRacialBody(racialBody), racialBody);
		}
		for(RaceStage raceStage : RaceStage.values()) {
			api.put("RACE_STAGE_"+raceStage.toString(), raceStage);
		}
		for(AbstractSubspecies subspecies : Subspecies.getAllSubspecies()) {
			api.put("SUBSPECIES_"+Subspecies.getIdFromSubspecies(subspecies), subspecies);
		}
		for(LegConfiguration legConf : LegConfiguration.values()) {
			api.put("LEG_CONFIGURATION_"+legConf.toString(), legConf);
		}
		for(FootStructure footStructure : FootStructure.values()) {
			api.put("FOOT_STRUCTURE_"+footStructure.toString(), footStructure);
		}
		for(GenitalArrangement genArrangement : GenitalArrangement.values()) {
			api.put("GENITALS_"+genArrangement.toString(), genArrangement);
		}
		for(BodyMaterial material : BodyMaterial.values()) {
			api.put("BODY_MATERIAL_"+material.toString(), material);
		}
		for(AbstractBodyCoveringType bct : BodyCoveringType.getAllBodyCoveringTypes()) {
			api.put("BODY_COVERING_TYPE_"+BodyCoveringType.getIdFromBodyCoveringType(bct), bct);
		}
		for(BodyCoveringCategory coveringCategory : BodyCoveringCategory.values()) {
			api.put("BODY_COVERING_CATEGORY_"+coveringCategory, coveringCategory);
		}
		for(CoveringPattern pattern : CoveringPattern.values()) {
			api.put("COVERING_PATTERN_"+pattern.toString(), pattern);
		}
		for(CoveringModifier modifier : CoveringModifier.values()) {
			api.put("COVERING_MODIFIER_"+modifier.toString(), modifier);
		}
		for(NippleShape nippleShape : NippleShape.values()) {
			api.put("NIPPLE_SHAPE_"+nippleShape.toString(), nippleShape);
		}
		for(CumProduction cumProduction : CumProduction.values()) {
			api.put("CUM_PRODUCTION_"+cumProduction.toString(), cumProduction);
		}
		for(BodyPartTag bpt : BodyPartTag.values()) {
			api.put("BODY_PART_TAG_"+bpt.toString(), bpt);
		}
		for(PenetrationModifier penMod : PenetrationModifier.values()) {
			api.put("PENETRATION_MODIFIER_"+penMod.toString(), penMod);
		}
		for(OrificeModifier orificeMod : OrificeModifier.values()) {
			api.put("ORIFICE_MODIFIER_"+orificeMod.toString(), orificeMod);
		}
		for(TongueModifier tongueMod : TongueModifier.values()) {
			api.put("TONGUE_MODIFIER_"+tongueMod.toString(), tongueMod);
		}
		for(Muscle muscle : Muscle.values()) {
			api.put("MUSCLE_"+muscle.toString(), muscle);
		}
		for(BodySize bodySize : BodySize.values()) {
			api.put("BODY_SIZE_"+bodySize.toString(), bodySize);
		}
		for(BodyShape bodyShape : BodyShape.values()) {
			api.put("BODY_SHAPE_"+bodyShape.toString(), bodyShape);
		}
		// Types:
		for(AbstractFluidType fluidType : FluidType.getAllFluidTypes()) {
			api.put("FLUID_TYPE_"+FluidType.getIdFromFluidType(fluidType), fluidType);
		}
		for(AbstractAntennaType type : AntennaType.getAllAntennaTypes()) {
			api.put("ANTENNA_TYPE_"+AntennaType.getIdFromAntennaType(type), type);
		}
		for(AbstractAnusType type : AnusType.getAllAnusTypes()) {
			api.put("ANUS_TYPE_"+AnusType.getIdFromAnusType(type), type);
		}
		for(AbstractArmType type : ArmType.getAllArmTypes()) {
			api.put("ARM_TYPE_"+ArmType.getIdFromArmType(type), type);
		}
		for(AbstractAssType type : AssType.getAllAssTypes()) {
			api.put("ASS_TYPE_"+AssType.getIdFromAssType(type), type);
		}
		for(AbstractBreastType type : BreastType.getAllBreastTypes()) {
			api.put("BREAST_TYPE_"+BreastType.getIdFromBreastType(type), type);
		}
		for(AbstractEarType type : EarType.getAllEarTypes()) {
			api.put("EAR_TYPE_"+EarType.getIdFromEarType(type), type);
		}
		for(AbstractEyeType type : EyeType.getAllEyeTypes()) {
			api.put("EYE_TYPE_"+EyeType.getIdFromEyeType(type), type);
		}
		for(AbstractFaceType type : FaceType.getAllFaceTypes()) {
			api.put("FACE_TYPE_"+FaceType.getIdFromFaceType(type), type);
		}
		for(AbstractFootType type : FootType.getAllFootTypes()) {
			api.put("FOOT_TYPE_"+FootType.getIdFromFootType(type), type);
		}
		for(AbstractHairType type : HairType.getAllHairTypes()) {
			api.put("HAIR_TYPE_"+HairType.getIdFromHairType(type), type);
		}
		for(AbstractHornType type : HornType.getAllHornTypes()) {
			api.put("HORN_TYPE_"+HornType.getIdFromHornType(type), type);
		}
		for(AbstractLegType type : LegType.getAllLegTypes()) {
			api.put("LEG_TYPE_"+LegType.getIdFromLegType(type), type);
		}
		for(AbstractMouthType type : MouthType.getAllMouthTypes()) {
			api.put("MOUTH_TYPE_"+MouthType.getIdFromMouthType(type), type);
		}
		for(AbstractNippleType type : NippleType.getAllNippleTypes()) {
			api.put("NIPPLE_TYPE_"+NippleType.getIdFromNippleType(type), type);
		}
		for(AbstractPenisType type : PenisType.getAllPenisTypes()) {
			api.put("PENIS_TYPE_"+PenisType.getIdFromPenisType(type), type);
		}
		for(AbstractTorsoType type : TorsoType.getAllTorsoTypes()) {
			api.put("TORSO_TYPE_"+TorsoType.getIdFromTorsoType(type), type);
		}
		for(AbstractTailType type : TailType.getAllTailTypes()) {
			api.put("TAIL_TYPE_"+TailType.getIdFromTailType(type), type);
		}
		for(AbstractTentacleType type : TentacleType.getAllTentacleTypes()) {
			api.put("TENTACLE_TYPE_"+TentacleType.getIdFromTentacleType(type), type);
		}
		for(AbstractTesticleType type : TesticleType.getAllTesticleTypes()) {
			api.put("TESTICLE_TYPE_"+TesticleType.getIdFromTesticleType(type), type);
		}
		for(AbstractTongueType type : TongueType.getAllTongueTypes()) {
			api.put("TONGUE_TYPE_"+TongueType.getIdFromTongueType(type), type);
		}
		for(AbstractVaginaType type : VaginaType.getAllVaginaTypes()) {
			api.put("VAGINA_TYPE_"+VaginaType.getIdFromVaginaType(type), type);
		}
		for(AbstractWingType type : WingType.getAllWingTypes()) {
			api.put("WING_TYPE_"+WingType.getIdFromWingType(type), type);
		}
		for(WingSize size : WingSize.values()) {
			api.put("WING_SIZE_"+size.toString(), size.getValue());
		}


		// Effects & persona:
		for(Fetish f : Fetish.values()) {
			api.put(f.toString(), f);
		}
		for(FetishDesire fetishDesire : FetishDesire.values()) {
			api.put("FETISH_DESIRE_"+fetishDesire.toString(), fetishDesire);
		}
		for(PersonalityTrait personalityTrait : PersonalityTrait.values()) {
			api.put("PERSONALITY_TRAIT_"+personalityTrait.toString(), personalityTrait);
		}
		for(Occupation occ : Occupation.values()) {
			api.put("OCCUPATION_"+occ.toString(), occ);
		}
		for (OccupationTag occupationTag : OccupationTag.values()) {
			api.put("OCCUPATION_TAG_" + occupationTag.toString(), occupationTag);
		}
		for(AbstractPerk p : Perk.getAllPerks()) {
			api.put("PERK_"+Perk.getIdFromPerk(p), p);
		}
		for(AbstractStatusEffect sa : StatusEffect.getAllStatusEffects()) {
			api.put("SE_"+StatusEffect.getIdFromStatusEffect(sa), sa);
		}
		for(AbstractAttribute att : Attribute.getAllAttributes()) {
			api.put("ATTRIBUTE_"+Attribute.getIdFromAttribute(att), att);
		}
		for(CorruptionLevel corruption : CorruptionLevel.values()) {
			api.put("CORRUPTION_LEVEL_"+corruption.toString(), corruption);
		}
		
		// Combat:
		for(DamageType damageType : DamageType.values()) {
			api.put("DAMAGE_TYPE_"+damageType.toString(), damageType);
		}
		for(SpellSchool spellSchool : SpellSchool.values()) {
			api.put("SPELL_SCHOOL_"+spellSchool.toString(), spellSchool);
		}
		for(Spell spell: Spell.values()) {
			api.put("SPELL_"+spell.toString(), spell);
		}
		for(SpellUpgrade spellUpgrade: SpellUpgrade.values()) {
			api.put("SPELL_UPGRADE_"+spellUpgrade.toString(), spellUpgrade);
		}
		for(CombatBehaviour behaviour: CombatBehaviour.values()) {
			api.put("COMBAT_BEHAVIOUR_"+behaviour.toString(), behaviour);
		}

		// Sex:
		for(SexParticipantType particiantType : SexParticipantType.values()) {
			api.put("SEX_PT_"+particiantType.toString(), particiantType);
		}
		for(SexAreaOrifice orifice : SexAreaOrifice.values()) {
			api.put("ORIFICE_"+orifice.toString(), orifice);
		}
		for(SexAreaPenetration penetration : SexAreaPenetration.values()) {
			api.put("PENETRATION_"+penetration.toString(), penetration);
		}
		for(GenericSexFlag flag : GenericSexFlag.values()) {
			api.put("SEX_FLAG_"+flag.toString(), flag);
		}

		// Other:
		for(Season season : Season.values()) {
			api.put("SEASON_"+season.toString(), season);
		}
		for(Weather w : Weather.values()) {
			api.put("WEATHER_"+w.toString(), w);
		}
		...
		for(SlavePermissionSetting permission : SlavePermissionSetting.values()) {
			api.put("SLAVE_PERMISSION_SETTING_"+permission.toString(), permission);
		}
		for(QuestLine questLine : QuestLine.values()) {
			api.put("QUEST_LINE_"+questLine.toString(), questLine);
		}
		for(Quest quest : Quest.values()) {
			api.put("QUEST_"+quest.toString(), quest);
		}
		for(SexualOrientation orientation : SexualOrientation.values()) {
			api.put("ORIENTATION_"+orientation.toString(), orientation);
		}
		for(Femininity femininity : Femininity.values()) {
			api.put("FEMININITY_"+femininity.toString(), femininity);
		}
		for(AffectionLevel affectionLevel : AffectionLevel.values()) {
			api.put("AFFECTION_"+affectionLevel.toString(), affectionLevel);
		}
		for(AffectionLevelBasic affectionLevelBasic : AffectionLevelBasic.values()) {
			api.put("AFFECTION_BASIC_"+affectionLevelBasic.toString(), affectionLevelBasic);
		}
		for(ObedienceLevel obedienceLevel : ObedienceLevel.values()) {
			api.put("OBEDIENCE_"+obedienceLevel.toString(), obedienceLevel);
		}
		for(ObedienceLevelBasic obedienceLevelBasic : ObedienceLevelBasic.values()) {
			api.put("OBEDIENCE_BASIC_"+obedienceLevelBasic.toString(), obedienceLevelBasic);
		}
		for(Relationship relationship : Relationship.values()) {
			api.put("RELATIONSHIP_"+relationship.toString(), relationship);
		}
		for(FurryPreference furryPreference : FurryPreference.values()) {
			api.put("FURRY_PREF_"+furryPreference.toString(), furryPreference);
		}
		for(ForcedTFTendency tfTendency : ForcedTFTendency.values()) {
			api.put("FORCED_TF_"+tfTendency.toString(), tfTendency);
		}
		for(ForcedFetishTendency fetishTendency : ForcedFetishTendency.values()) {
			api.put("FORCED_FETISH_"+fetishTendency.toString(), fetishTendency);
		}
		for(AbstractWorldType worldType : WorldType.getAllWorldTypes()) {
			api.put("WORLD_TYPE_"+WorldType.getIdFromWorldType(worldType), worldType);
		}
		for(AbstractPlaceType placeType : PlaceType.getAllPlaceTypes()) {
			api.put("PLACE_TYPE_"+PlaceType.getIdFromPlaceType(placeType), placeType);
		}
...
//		});
	...
	public static void setClothingTypeForParsing(AbstractClothingType clothingTypeForParsing) {
		...
		api.put("clothing", getClothingTypeForParsing());
	}
